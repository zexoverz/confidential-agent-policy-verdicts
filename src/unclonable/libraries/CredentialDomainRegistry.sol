...
contract CredentialDomainRegistry {
    ...
    function registerDomain(uint256 homeDomainId) external {
        require(!domains[homeDomainId].registered, "CCC: domain already registered");
    ...
    function revokeDomain(uint256 homeDomainId) external {
        require(domains[homeDomainId].registered, "CCC: domain not registered");
        require(!domains[homeDomainId].revoked, "CCC: domain already revoked");
