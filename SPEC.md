...
A Policy Domain registers a commitment to its ruleset as a statement in the [ERC-7812](./erc-7812.md) Evidence Registry. An off-chain policy engine evaluates a candidate action against the ruleset and emits a proof whose public inputs bind the verdict to an [ERC-8004](./erc-8004.md) agent identity, the [ERC-7812](./erc-7812.md) policy root the decision was made against, a commitment to the action, the address permitted to execute it, an expiry, and a single-use nullifier. A guard contract on any EVM chain verifies that proof locally and gates execution on it.
...
The first is **retrospective**: [ERC-8004](./erc-8004.md) records identity, reputation, and validation attestations after an agent has acted. This is useful for counterparty selection and useless for interdiction.

The second is **mandate-based**: [ERC-8150](./erc-8150.md) verifies, before execution, that an agent's batch matches an intent the user signed in advance. [ERC-8312](./erc-8312.md) meters how much of a bounded mandate an agent has spent. Both derive authority from a principal who signs a specific grant.
...
Zero knowledge resolves the tension: the verifier learns that *some committed policy* was correctly evaluated and returned allow, and learns nothing about the policy's contents. [ERC-7812](./erc-7812.md) already supplies the missing half, since it defines a registry of blinded statements whose state can be proven in zero knowledge, and it is written abstractly so that later ERCs can build specific use cases on top of it. Nothing in the agent cluster has taken it up.

This ERC is that bridge: [ERC-8004](./erc-8004.md) for who the agent is, [ERC-7812](./erc-7812.md) for what the policy commits to, and a verdict envelope in between.
...
- **Policy Commitment**: a hash of the ruleset, registered as a statement in the [ERC-7812](./erc-7812.md) Evidence Registry through a Registrar.
- **Policy Root**: the [ERC-7812](./erc-7812.md) `EvidenceDB` SMT root containing the current Policy Commitment.
...
A Policy Domain MUST register its Policy Commitment through an [ERC-7812](./erc-7812.md) Registrar as a key/value statement:
...
1. The private ruleset hashes to a `policyCommitment` that is included in `policyRoot`, verified by an [ERC-7812](./erc-7812.md) inclusion proof against the `EvidenceDB` structure under `H(domainId, version)`.
...
`agentId` MUST be a valid token id in the [ERC-8004](./erc-8004.md) Identity Registry **on the chain where the verdict is consumed**. Cross-chain identity resolution is out of scope for this standard and is deferred to a future ERC. A Guard MUST reject a verdict for a non-existent `agentId`.

A Guard SHOULD, after a successful `consume`, write an attestation to the [ERC-8004](./erc-8004.md) Validation Registry recording that `agentId` satisfied `domainId` at `policyRoot`. This exposes the fact of compliance to the public reputation layer while disclosing nothing about the policy. The attestation payload MUST NOT contain the ruleset or any part of it, and SHOULD have the following shape:
...
Where a domain also meters authority, a Guard SHOULD decrement the [ERC-8312](./erc-8312.md) bounded mandate for `agentId` after `consume` succeeds and before executing the action. A verdict and a mandate are independent conditions and both MUST hold. This standard defines permission, [ERC-8312](./erc-8312.md) defines remaining room, and neither subsumes the other.
...
By proving a fixed **interpreter** and passing the ruleset as witness, `programKey` stays constant across policy updates. Only the [ERC-7812](./erc-7812.md) root moves. This is why the specification insists the program be an interpreter, and it is why the standard is realistic to operate at all.
...
- `ConfidentialPolicyVerdict` Guard (`verify` / `consume` / `isConsumed`) with the checks in the
  order specified, and a `PolicyDomainRegistry` companion (root rotation with `maxRootAge` grace,
  immediate revocation).
- A proving-system-agnostic `IVerifier` boundary (`verifyProof(programKey, publicInputs, proof)`)
  so SP1 / Groth16 / RISC0 slot in behind one interface.
- `PolicyAction` — the canonical, domain-separated (`chainId` + `domainId`) action-commitment
...
- The SP1 interpreter program and its Groth16 wrapper for on-chain verification (skeleton present,
  documenting the in-circuit obligations).
...
This ERC requires [ERC-7812](./erc-7812.md) (Review), [ERC-8004](./erc-8004.md), and [ERC-8312](./erc-8312.md), all recent. A Standards Track ERC MAY sit in Draft or Review atop pre-Final dependencies, but it cannot advance to Final until they do. Integrators SHOULD pin the exact versions of the dependencies they rely on.

---

# ERC-XXXX: Confidential Agent Execution Credentials (Companion)
...
CREDENTIAL_TAG = keccak256("ERC-XXXX/credential/v1")
```
...
capabilityCommitment = H(CREDENTIAL_TAG, salt, agentId, homeChainId,
